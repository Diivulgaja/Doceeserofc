...
const STATUS_UI = {
  novo: { 
    label: "Recebido", 
    message: "Seu pagamento foi confirmado! A cozinha já vai começar.", 
    icon: Check, 
    color: "text-blue-600", 
    bg: "bg-blue-100",
    bar: "w-[10%]"
...
export default function App() {
  ...
  const logoutUser = () => {
    ...
    setCustomer({ nome: '', telefone: '', email: '', cpf: '', rua: '', numero: '', bairro: '' });
  ...
  const createAbacateCharge = async () => {
    ...
    try {
      let cleanPhone = customer.telefone.replace(/\D/g, '');
  ...
  const PaymentModal = () => {
    ...
    // Se tiver copypaste, tenta usar. Se não, fallback
    ...
    const fetchOrders = async (phone) => {
      ...
      try {
        const { data, error } = await supabase
          .from(COLLECTION_ORDERS)
          .select('*')
          .ilike('customer->>telefone', `%${phone.replace(/\D/g, '')}%`)
